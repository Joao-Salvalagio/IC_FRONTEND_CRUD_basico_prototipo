<template>
  ...
  <Transition name="fade"
    ><ConfirmDialog
      ...
      message="Tem certeza que deseja excluir esta GPU?"
  ...
  <main class="manager-view" :class="{ 'modal-open': isModalVisible || showConfirmDialog }">
    <header class="page-header">
      <div class="title">
        <div class="title-text">
          <h1>Gerenciar Placas de Vídeo (GPU)</h1>
          <p>Adicione, edite ou remova GPUs da base de dados.</p>
        </div>
      </div>
      <div class="actions">
    ...
    <section class="user-list-container">
      <div class="list-header">
        <h2>Lista de Placas de Vídeo</h2>
...
async function handleSaveItem(data) {
  try {
    ...
    const url = isUpdating
      ? `http://localhost:8080/api/gpus/${data.id}`
      : 'http://localhost:8080/api/gpus'
    const method = isUpdating ? 'PUT' : 'POST'
    const payload = isUpdating
      ? data
      : { nome: data.nome, marca: data.marca, memoriaVram: data.memoriaVram, preco: data.preco }
...
async function confirmDeletion() {
  try {
    const response = await fetch(`http://localhost:8080/api/gpus/${itemToDeleteId.value}`, {
      method: 'DELETE',
    })
