  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group"><label for="potenciaWatts">Potência (Watts)</label><input id="potenciaWatts" v-model.number="formData.potenciaWatts" type="number" required /></div>
        <div class="form-group"><label for="formato">Formato (ATX, SFX...)</label><input id="formato" v-model="formData.formato" type="text" required /></div>
...
const props = defineProps({ fonte: { type: Object, default: null } });
// formData ATUALIZADO
const formData = ref({ id: null, nome: '', marca: '', potenciaWatts: 0, formato: '', preco: 0.0 });
const modalTitle = computed(() => props.fonte ? 'Editar Fonte' : 'Adicionar Nova Fonte');
watch(() => props.fonte, (newVal) => {
  formData.value = newVal ? { ...newVal } : { id: null, nome: '', marca: '', potenciaWatts: 0, formato: '', preco: 0.0 };
}, { immediate: true });
