<template>
  ...
  <Transition name="fade"
    ><ConfirmDialog
      ...
      message="Tem certeza que deseja excluir esta fonte?"
  ...
  <main class="manager-view" :class="{ 'modal-open': isModalVisible || showConfirmDialog }">
    <header class="page-header">
      <div class="title">
        <div class="title-text">
          <h1>Gerenciar Fontes (PSU)</h1>
          <p>Adicione, edite ou remova fontes de alimentação da base de dados.</p>
        </div>
      </div>
      <div class="actions">
    ...
    <section class="user-list-container">
      <div class="list-header">
        <h2>Lista de Fontes de Alimentação</h2>
      ...
      <table class="user-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="item in items" :key="item.id">
            ...
            <td>{{ item.marca }}</td>
            <td>
              <span>{{ item.potenciaWatts }} W</span>
            </td>
            <td>
...
async function handleSaveItem(data) {
  try {
    ...
    const url = isUpdating
      ? `http://localhost:8080/api/fontes/${data.id}`
      : 'http://localhost:8080/api/fontes'
    ...
    const payload = isUpdating
      ...
      : {
          nome: data.nome,
          marca: data.marca,
          potenciaWatts: data.potenciaWatts,
...
async function confirmDeletion() {
  try {
    const response = await fetch(`http://localhost:8080/api/fontes/${itemToDeleteId.value}`, {
      method: 'DELETE',
    })
