<template>
  ...
  <Transition name="fade"
    ><ConfirmDialog
      ...
      message="Tem certeza que deseja excluir este gabinete?"
    ...
    <header class="page-header">
      <div class="title">
        <div class="title-text">
          ...
          <p>Adicione, edite ou remova gabinetes da base de dados.</p>
        </div>
      </div>
      <div class="actions">
      ...
      <table class="user-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="item in items" :key="item.id">
            ...
            <td>{{ item.marca }}</td>
            <td>
              <span>{{ item.formatosPlacaMaeSuportados }}</span>
            </td>
...
async function handleSaveItem(data) {
  try {
    ...
    const url = isUpdating
      ? `http://localhost:8080/api/gabinetes/${data.id}`
      : 'http://localhost:8080/api/gabinetes'
    ...
    const payload = isUpdating
      ...
      : {
          nome: data.nome,
          marca: data.marca,
          formatosPlacaMaeSuportados: data.formatosPlacaMaeSuportados,
          preco: data.preco,
        }
...
async function confirmDeletion() {
  try {
    const response = await fetch(`http://localhost:8080/api/gabinetes/${itemToDeleteId.value}`, {
      method: 'DELETE',
    })
