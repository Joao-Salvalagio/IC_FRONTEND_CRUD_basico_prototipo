<template>
  <Transition name="fade"
    ><RefrigeracaoFormModal
      v-if="isModalVisible"
      :refrigeracao="itemToEdit"
      @close="closeModal"
      @save="handleSaveItem"
  /></Transition>
  <Transition name="fade"
    ><ConfirmDialog
      ...
      message="Tem certeza que deseja excluir este cooler?"
    ...
    <header class="page-header">
      <div class="title">
        <div class="title-text">
          <h1>Gerenciar Refrigeração (Cooler)</h1>
          <p>Adicione, edite ou remova coolers da base de dados.</p>
        </div>
      </div>
      <div class="actions">
      ...
      <table class="user-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="item in items" :key="item.id">
            ...
            <td>
              <span>{{ item.soquetesCpuSuportados }}</span>
            </td>
...
async function handleSaveItem(data) {
  try {
    ...
    const url = isUpdating
      ? `http://localhost:8080/api/refrigeracoes/${data.id}`
      : 'http://localhost:8080/api/refrigeracoes'
    ...
    const payload = isUpdating
      ...
      : {
          ...
          marca: data.marca,
          tipo: data.tipo,
          soquetesCpuSuportados: data.soquetesCpuSuportados,
          preco: data.preco,
        }
...
async function confirmDeletion() {
  try {
    const response = await fetch(
      `http://localhost:8080/api/refrigeracoes/${itemToDeleteId.value}`,
      { method: 'DELETE' },
    )
