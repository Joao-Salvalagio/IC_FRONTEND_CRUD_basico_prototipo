  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          <label for="potencia">Potência Recomendada (W)</label>
          <input id="potencia" v-model.number="formData.potenciaRecomendadaW" type="number" required />
        </div>
        <div class="form-group">
...
// formData INICIAL AGORA INCLUI O NOVO CAMPO
const formData = ref({ id: null, nome: '', marca: '', soquete: '', potenciaRecomendadaW: 0, preco: 0.0 });

const modalTitle = computed(() => {
  return props.cpu ? 'Editar Processador' : 'Adicionar Novo Processador';
});

watch(() => props.cpu, (newCpu) => {
  if (newCpu) {
    formData.value = { ...newCpu };
  } else {
    // formData RESETADO TAMBÉM INCLUI O NOVO CAMPO
    formData.value = { id: null, nome: '', marca: '', soquete: '', potenciaRecomendadaW: 0, preco: 0.0 };
  }
}, { immediate: true });
