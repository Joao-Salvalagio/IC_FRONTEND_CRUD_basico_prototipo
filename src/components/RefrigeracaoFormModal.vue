  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="form-group"><label for="tipo">Tipo (Air Cooler, Water Cooler...)</label><input id="tipo" v-model="formData.tipo" type="text" required /></div>
        <div class="form-group"><label for="soquetesCpuSuportados">Soquetes de CPU Suportados</label><input id="soquetesCpuSuportados" v-model="formData.soquetesCpuSuportados" type="text" required placeholder="Ex: AM4, AM5, LGA1700" /></div>
...
const props = defineProps({ refrigeracao: { type: Object, default: null } });
// formData ATUALIZADO
const formData = ref({ id: null, nome: '', marca: '', tipo: '', soquetesCpuSuportados: '', preco: 0.0 });
const modalTitle = computed(() => props.refrigeracao ? 'Editar Cooler' : 'Adicionar Novo Cooler');
watch(() => props.refrigeracao, (newVal) => {
  formData.value = newVal ? { ...newVal } : { id: null, nome: '', marca: '', tipo: '', soquetesCpuSuportados: '', preco: 0.0 };
}, { immediate: true });
