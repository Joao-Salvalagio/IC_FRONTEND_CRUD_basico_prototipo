<template>
  ...
  <Transition name="fade"
    ><ConfirmDialog
      ...
      message="Tem certeza que deseja excluir esta Memória RAM?"
    ...
    <header class="page-header">
      <div class="title">
        <div class="title-text">
          <h1>Gerenciar Memórias RAM</h1>
          <p>Adicione, edite ou remova memórias da base de dados.</p>
        </div>
      </div>
      <div class="actions">
      ...
      <table class="user-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="item in items" :key="item.id">
            ...
            <td>
              <span>{{ item.frequenciaMhz }} MHz</span>
            </td>
            <td>
              <span>{{ item.tipo }}</span>
            </td>
...
async function handleSaveItem(data) {
  try {
    ...
    const url = isUpdating
      ? `http://localhost:8080/api/memorias-ram/${data.id}`
      : 'http://localhost:8080/api/memorias-ram'
...
async function confirmDeletion() {
  try {
    const response = await fetch(`http://localhost:8080/api/memorias-ram/${itemToDeleteId.value}`, {
      method: 'DELETE',
    })
