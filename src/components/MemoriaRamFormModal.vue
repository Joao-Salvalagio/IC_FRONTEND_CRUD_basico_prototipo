  ...
  <div class="modal-overlay" @click.self="$emit('close')">
    <div class="modal-content">
      ...
      <form @submit.prevent="handleSubmit">
        <div class="form-group"><label for="nome">Nome</label><input id="nome" v-model="formData.nome" type="text" required /></div>
        <div class="form-group"><label for="marca">Marca</label><input id="marca" v-model="formData.marca" type="text" required /></div>
        <div class="form-group"><label for="capacidadeGb">Capacidade (GB)</label><input id="capacidadeGb" v-model.number="formData.capacidadeGb" type="number" required /></div>
        <div class="form-group"><label for="frequenciaMhz">Frequência (MHz)</label><input id="frequenciaMhz" v-model.number="formData.frequenciaMhz" type="number" required /></div>
        <div class="form-group"><label for="tipo">Tipo (DDR4, DDR5...)</label><input id="tipo" v-model="formData.tipo" type="text" required /></div>
...
const props = defineProps({ memoriaRam: { type: Object, default: null } });
const formData = ref({ id: null, nome: '', marca: '', capacidadeGb: 0, frequenciaMhz: 0, tipo: '', preco: 0.0 });
const modalTitle = computed(() => props.memoriaRam ? 'Editar Memória RAM' : 'Adicionar Nova Memória RAM');
watch(() => props.memoriaRam, (newVal) => {
  formData.value = newVal ? { ...newVal } : { id: null, nome: '', marca: '', capacidadeGb: 0, frequenciaMhz: 0, tipo: '', preco: 0.0 };
}, { immediate: true });
...
<style scoped>
/* CSS idêntico aos outros modais */
