...
// --- FUNÇÃO handleSaveUser ATUALIZADA ---
async function handleSaveUser(userData) {
  try {
    ...
    const payload = {
      ...
      // (Se for edição e a senha estiver vazia, não a enviamos)
      ...(userData.password && { password: userData.password })
    };

    // Se for CRIAÇÃO, a senha é obrigatória
    if (!isUpdating && !payload.password) {
        alertMessage.value = "O campo Senha é obrigatório para criar um novo usuário.";
